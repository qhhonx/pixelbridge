...
Transfers and cleanup share **Minimum free space on Pixel** in Settings. Before each transfer, PixelBridge budgets this reserve plus the file and other concurrent transfers. For example, a 1 GB reserve and a 600 MB file require at least 1.6 GB free. If space is insufficient, it stops all transfers and persists a cleanup hold and the pending-file budget across pauses and restarts. Waiting for upload completion, temperature, an available foreground screen, an empty cleanup page or the post-action cooldown never admits new transfers.

PixelBridge operates Google Photos’ **Free up space on this device**, requiring recognized backup-complete and safety messages. After completion it remeasures space; transfers resume only when the reserve and pending-file budget can be met. External/manual space recovery may also release the hold, but any already-started cleanup must first be reconciled. Disabling the experiment explicitly cancels an unstarted cleanup hold; an uncertain started operation is still checked before transfers. Completed transfer records remain intact. No direct deletion of photo folders is performed.
