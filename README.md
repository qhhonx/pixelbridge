...
In **Experiments**, pause backup and choose **Check and enable** under **Auto-free Pixel space**. This checks the connected device, reads the current Google Photos account, and navigates to the official device-cleanup confirmation or empty page. It stores only an account fingerprint and does not clean during setup. An empty page verifies navigation; every actual cleanup still requires the official safe-backup confirmation. It is off by default and must be enabled separately on each Mac.

Transfers and cleanup share **Minimum free space on Pixel** in Settings. Before each transfer, PixelBridge budgets this reserve plus the file and other concurrent transfers. For example, a 1 GB reserve and a 600 MB file require at least 1.6 GB free. If space is insufficient, it stops all transfers and persists a cleanup hold and the pending-file budget across pauses and restarts. Waiting for eligible copies, temperature, an available foreground screen, an empty cleanup page or the post-action cooldown never admits new transfers.

PixelBridge operates Google Photos’ **Free up space on this device**, requiring an explicit safety message that the selected copies are backed up. It does not require all uploads to finish; Google Photos selects eligible copies. After completion it remeasures space; transfers resume only when the reserve and pending-file budget can be met. External/manual space recovery may also release the hold, but any already-started cleanup must first be reconciled. Disabling the experiment explicitly cancels an unstarted cleanup hold; an uncertain started operation is still checked before transfers. Completed transfer records remain intact. No direct deletion of photo folders is performed.
