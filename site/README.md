...
For a private repository, grant the Vercel GitHub App access to this repository before connecting it. Keep repository access scoped to selected repositories. After the first push, verify that the production deployment reports the same commit as `main`. The root `.vercelignore` also restricts manual CLI uploads to website files; local app builds, photo state and signing keys are excluded.
