...
实况照片保留动态，连拍保留整组瞬间。PixelBridge 将支持的实况照片封装为动态照片，并为连拍副本保留分组与代表帧信息。Apple 的 HEVC 转换器拒绝损坏的配对视频时，如 Mac 已安装 FFmpeg，可恢复可解码帧再封装；原始文件不变。连拍每帧独立传输与校验。JPEG 分组已在 Pixel 和 Google Photos 网页验证，HEIC 云端效果仍待验证。
